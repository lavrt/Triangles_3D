...
protected:
    ...
    Triangle<double> triangle = Triangle<double>(p0, p1, p2);
};
...
TEST(TriangleIntersectionTest, PerpendicularIntersectingTriangles) {
    Triangle<double> perpendicular(Point<double>{0.2,0.2,0.5}, Point<double>{0.2,0.2,-0.5}, Point<double>{0.8,0.8,0});
    Triangle<double> xy_plane(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
...
TEST(ComputeBoundingBoxTest, SingleTriangle) {
    Triangle<double> t(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{0,3,0});
...
TEST(ComputeBoundingBoxTest, MultipleTriangles) {
    std::vector<Triangle<double>> triangles = {
        ...
        Triangle<double>(Point<double>{2,2,1}, Point<double>{3,2,1}, Point<double>{2,3,1}),
        Triangle<double>(Point<double>{-1,-1,-1}, Point<double>{0,-1,-1}, Point<double>{-1,0,-1})
    };
...
TEST(TriangleEdgeCasesTest, ContainsMethod) {
    Triangle<double> large(Point<double>{0,0,0}, Point<double>{3,0,0}, Point<double>{0,3,0});
    Triangle<double> small(Point<double>{0.5,0.5,0}, Point<double>{1.5,0.5,0}, Point<double>{0.5,1.5,0});
...
TEST(TriangleDegenerateTest, PointTriangleIntersection) {
    Triangle<double> point_triangle(Point<double>{0,0,0}, Point<double>{0,0,0}, Point<double>{0,0,0});
    Triangle<double> normal_triangle(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
...
TEST(TriangleDegenerateTest, LineSegmentTriangleIntersection) {
    Triangle<double> line_triangle(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0.5,0,0});
    Triangle<double> normal_triangle(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
...
TEST(TriangleDegenerateTest, TwoPointTrianglesNoIntersection) {
    Triangle<double> point1(Point<double>{0,0,0}, Point<double>{0,0,0}, Point<double>{0,0,0});
    Triangle<double> point2(Point<double>{1,1,1}, Point<double>{1,1,1}, Point<double>{1,1,1});
    
    EXPECT_FALSE(Triangle<double>::Intersect(point1, point2));
}

TEST(TriangleDegenerateTest, LineSegmentAndPointIntersection) {
    Triangle<double> line_triangle(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> point_triangle(Point<double>{1,0,0}, Point<double>{1,0,0}, Point<double>{1,0,0});
...
TEST(TriangleDegenerateTest, CollinearLineSegmentsIntersection) {
    Triangle<double> line1(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> line2(Point<double>{1,0,0}, Point<double>{3,0,0}, Point<double>{2,0,0});
...
TEST(TriangleDegenerateTest, ParallelLineSegmentsNoIntersection) {
    Triangle<double> line1(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> line2(Point<double>{0,1,0}, Point<double>{2,1,0}, Point<double>{1,1,0});
...
TEST(TriangleDegenerateTest, DegenerateTriangleContainedInNormalTriangle) {
    Triangle<double> degenerate(Point<double>{0.2,0.2,0}, Point<double>{0.8,0.2,0}, Point<double>{0.5,0.2,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
...
TEST(TriangleDegenerateTest, DegenerateTriangleOnEdgeOfNormalTriangle) {
    Triangle<double> point_triangle(Point<double>{0.5,0,0}, Point<double>{0.5,0,0}, Point<double>{0.5,0,0});
    Triangle<double> normal_triangle(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
...
TEST(DegenerateSegmentNormalTest, SegmentInsideNormalTriangle) {
    Triangle<double> segment(Point<double>{0.3,0.3,0}, Point<double>{0.7,0.3,0}, Point<double>{0.5,0.3,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(segment, normal));
}

TEST(DegenerateSegmentNormalTest, SegmentThroughNormalTriangle) {
    Triangle<double> segment(Point<double>{0.5,-0.5,0}, Point<double>{0.5,1.5,0}, Point<double>{0.5,0.5,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(segment, normal));
}

TEST(DegenerateSegmentNormalTest, SegmentOutsideNormalTriangle) {
    Triangle<double> segment(Point<double>{1.5,1.5,0}, Point<double>{2.5,1.5,0}, Point<double>{2.0,1.5,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
...
TEST(DegenerateSegmentNormalTest, SegmentTouchingNormalTriangleVertex) {
    Triangle<double> segment(Point<double>{1,0,0}, Point<double>{2,0,0}, Point<double>{1.5,0,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(segment, normal));
}

TEST(DegenerateSegmentNormalTest, SegmentAlongNormalTriangleEdge) {
    Triangle<double> segment(Point<double>{0,0,0}, Point<double>{0.5,0,0}, Point<double>{0.25,0,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(segment, normal));
}

TEST(DegenerateSegmentNormalTest, SegmentInDifferentPlaneNoIntersection) {
    Triangle<double> segment(Point<double>{0.3,0.3,1}, Point<double>{0.7,0.3,1}, Point<double>{0.5,0.3,1});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_FALSE(Triangle<double>::Intersect(segment, normal));
}

TEST(DegeneratePointNormalTest, PointInsideNormalTriangle) {
    Triangle<double> point(Point<double>{0.3,0.3,0}, Point<double>{0.3,0.3,0}, Point<double>{0.3,0.3,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(point, normal));
}

TEST(DegeneratePointNormalTest, PointOnNormalTriangleVertex) {
    Triangle<double> point(Point<double>{1,0,0}, Point<double>{1,0,0}, Point<double>{1,0,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(point, normal));
}

TEST(DegeneratePointNormalTest, PointOnNormalTriangleEdge) {
    Triangle<double> point(Point<double>{0.5,0,0}, Point<double>{0.5,0,0}, Point<double>{0.5,0,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(point, normal));
}

TEST(DegeneratePointNormalTest, PointOutsideNormalTriangle) {
    Triangle<double> point(Point<double>{1.5,1.5,0}, Point<double>{1.5,1.5,0}, Point<double>{1.5,1.5,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_FALSE(Triangle<double>::Intersect(point, normal));
}

TEST(DegeneratePointNormalTest, PointInDifferentPlaneNoIntersection) {
    Triangle<double> point(Point<double>{0.3,0.3,1}, Point<double>{0.3,0.3,1}, Point<double>{0.3,0.3,1});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_FALSE(Triangle<double>::Intersect(point, normal));
}

TEST(DegenerateSegmentSegmentTest, CollinearSegmentsOverlap) {
    Triangle<double> seg1(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> seg2(Point<double>{1,0,0}, Point<double>{3,0,0}, Point<double>{2,0,0});
    EXPECT_TRUE(Triangle<double>::Intersect(seg1, seg2));
}

TEST(DegenerateSegmentSegmentTest, CollinearSegmentsTouch) {
    Triangle<double> seg1(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0.5,0,0});
    Triangle<double> seg2(Point<double>{1,0,0}, Point<double>{2,0,0}, Point<double>{1.5,0,0});
    EXPECT_TRUE(Triangle<double>::Intersect(seg1, seg2));
}

TEST(DegenerateSegmentSegmentTest, CollinearSegmentsNoOverlap) {
    Triangle<double> seg1(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0.5,0,0});
    Triangle<double> seg2(Point<double>{2,0,0}, Point<double>{3,0,0}, Point<double>{2.5,0,0});
    EXPECT_FALSE(Triangle<double>::Intersect(seg1, seg2));
}

TEST(DegenerateSegmentSegmentTest, IntersectingSegments) {
    Triangle<double> seg1(Point<double>{0,0,0}, Point<double>{2,2,0}, Point<double>{1,1,0});
    Triangle<double> seg2(Point<double>{0,2,0}, Point<double>{2,0,0}, Point<double>{1,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(seg1, seg2));
}

TEST(DegenerateSegmentSegmentTest, ParallelSegmentsNoIntersection) {
    Triangle<double> seg1(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> seg2(Point<double>{0,1,0}, Point<double>{2,1,0}, Point<double>{1,1,0});
    EXPECT_FALSE(Triangle<double>::Intersect(seg1, seg2));
}

TEST(DegenerateSegmentSegmentTest, SegmentsInDifferentPlanes) {
    Triangle<double> seg1(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> seg2(Point<double>{0,0,1}, Point<double>{2,0,1}, Point<double>{1,0,1});
    EXPECT_FALSE(Triangle<double>::Intersect(seg1, seg2));
}

TEST(DegenerateSegmentPointTest, PointInsideSegment) {
    Triangle<double> segment(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> point(Point<double>{1,0,0}, Point<double>{1,0,0}, Point<double>{1,0,0});
...
TEST(DegenerateSegmentPointTest, PointOnSegmentEndPoint) {
    Triangle<double> segment(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> point(Point<double>{0,0,0}, Point<double>{0,0,0}, Point<double>{0,0,0});
    EXPECT_TRUE(Triangle<double>::Intersect(segment, point));
}

TEST(DegenerateSegmentPointTest, PointOutsideSegment) {
    Triangle<double> segment(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> point(Point<double>{3,0,0}, Point<double>{3,0,0}, Point<double>{3,0,0});
    EXPECT_FALSE(Triangle<double>::Intersect(segment, point));
}

TEST(DegenerateSegmentPointTest, PointNotOnSegmentLine) {
    Triangle<double> segment(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> point(Point<double>{1,1,0}, Point<double>{1,1,0}, Point<double>{1,1,0});
    EXPECT_FALSE(Triangle<double>::Intersect(segment, point));
}

TEST(DegenerateSegmentPointTest, SegmentAndPointInDifferentPlanes) {
    Triangle<double> segment(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{1,0,0});
    Triangle<double> point(Point<double>{1,0,1}, Point<double>{1,0,1}, Point<double>{1,0,1});
    EXPECT_FALSE(Triangle<double>::Intersect(segment, point));
}

TEST(DegeneratePointPointTest, SamePoints) {
    Triangle<double> point1(Point<double>{1,2,3}, Point<double>{1,2,3}, Point<double>{1,2,3});
    Triangle<double> point2(Point<double>{1,2,3}, Point<double>{1,2,3}, Point<double>{1,2,3});
    EXPECT_TRUE(Triangle<double>::Intersect(point1, point2));
}

TEST(DegeneratePointPointTest, DifferentPoints) {
    Triangle<double> point1(Point<double>{1,2,3}, Point<double>{1,2,3}, Point<double>{1,2,3});
    Triangle<double> point2(Point<double>{4,5,6}, Point<double>{4,5,6}, Point<double>{4,5,6});
    EXPECT_FALSE(Triangle<double>::Intersect(point1, point2));
}

TEST(DegeneratePointPointTest, CloseButDifferentPoints) {
    Triangle<double> point1(Point<double>{1,2,3}, Point<double>{1,2,3}, Point<double>{1,2,3});
    Triangle<double> point2(Point<double>{1.0001,2.0001,3.0001}, Point<double>{1.0001,2.0001,3.0001}, Point<double>{1.0001,2.0001,3.0001});
    EXPECT_FALSE(Triangle<double>::Intersect(point1, point2));
}

TEST(DegeneratePointPointTest, PointsInDifferentPlanes) {
    Triangle<double> point1(Point<double>{1,2,0}, Point<double>{1,2,0}, Point<double>{1,2,0});
    Triangle<double> point2(Point<double>{1,2,1}, Point<double>{1,2,1}, Point<double>{1,2,1});
    EXPECT_FALSE(Triangle<double>::Intersect(point1, point2));
}

TEST(DegenerateEdgeCasesTest, ZeroLengthSegment) {
    Triangle<double> zero_segment(Point<double>{1,1,1}, Point<double>{1,1,1}, Point<double>{1,1,1});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{0,2,0});
...
TEST(DegenerateEdgeCasesTest, SegmentExactlyOnTrianglePlaneButOutside) {
    Triangle<double> segment(Point<double>{2,2,0}, Point<double>{3,2,0}, Point<double>{2.5,2,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_FALSE(Triangle<double>::Intersect(segment, normal));
}

TEST(DegenerateEdgeCasesTest, PointOnTriangleBoundary) {
    Triangle<double> point(Point<double>{0.5,0.5,0}, Point<double>{0.5,0.5,0}, Point<double>{0.5,0.5,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{1,0,0}, Point<double>{0,1,0});
    EXPECT_TRUE(Triangle<double>::Intersect(point, normal));
}

TEST(DegenerateEdgeCasesTest, DegenerateTriangleWithNegativeCoordinates) {
    Triangle<double> segment(Point<double>{-1,-1,0}, Point<double>{1,1,0}, Point<double>{0,0,0});
    Triangle<double> normal(Point<double>{0,0,0}, Point<double>{2,0,0}, Point<double>{0,2,0});
    EXPECT_TRUE(Triangle<double>::Intersect(segment, normal));
}
