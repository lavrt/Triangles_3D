...
protected:
    const Vector v_1{1, 2, 3};
    const Vector v_2{4, 5, 6};
...
TEST_F(VectorTest, Constructor) {
    ...
    double expected_x = 1.5;
    double expected_y = 2.5;
    double expected_z = 3.5;

    EXPECT_DOUBLE_EQ(v.x, expected_x);
    EXPECT_DOUBLE_EQ(v.y, expected_y);
    EXPECT_DOUBLE_EQ(v.z, expected_z);
}

// Dot ---------------------------------------------------------------------------------------------

TEST_F(VectorTest, DotProduct) {
    double result = Vector::Dot(v_1, v_2);
...
TEST_F(VectorTest, DotProductCommutative) {
    double dot_1 = Vector::Dot(v_1, v_2);
    double dot_2 = Vector::Dot(v_2, v_1);

    EXPECT_DOUBLE_EQ(dot_1, dot_2);
}

TEST_F(VectorTest, DotProductWithZeroVector) {
    double result = Vector::Dot(v_zero, v_1);
...
TEST_F(VectorTest, CrossProduct) {
    Vector result = Vector::Cross(v_1, v_2);
...
TEST_F(VectorTest, CrossProductOrthogonal) {
    Vector cross = Vector::Cross(v_1, v_2);

    EXPECT_DOUBLE_EQ(Vector::Dot(cross, v_1), 0.0);
    EXPECT_DOUBLE_EQ(Vector::Dot(cross, v_2), 0.0);
}

TEST_F(VectorTest, CrossProductAnticommutative) {
    Vector cross_1 = Vector::Cross(v_1, v_2);
    Vector cross_2 = Vector::Cross(v_2, v_1);
...
TEST_F(VectorTest, CrossProductWithZeroVector) {
    Vector result = Vector::Cross(v_zero, v_1);
...
TEST_F(VectorTest, CrossProductParallelVectors) {
    Vector parallel_v_1{2.0, 4.0, 6.0};
    Vector result = Vector::Cross(v_1, parallel_v_1);
...
TEST_F(VectorTest, NormalizedVector) {
    Vector normalized = v_1.Normalized();
    double length = normalized.Length();
    
    EXPECT_DOUBLE_EQ(length, 1.0);
    
    double ratio_x = normalized.x / v_1.x;
    double ratio_y = normalized.y / v_1.y;
    double ratio_z = normalized.z / v_1.z;
...
TEST_F(VectorTest, CollinearVectors) {
    Vector v_1_doubled{2, 4, 6};

    EXPECT_TRUE(v_1.Collinear(v_1_doubled));
    EXPECT_TRUE(v_1_doubled.Collinear(v_1));
}

TEST_F(VectorTest, NonCollinearVectors) {
    EXPECT_FALSE(v_1.Collinear(v_2));
    EXPECT_FALSE(v_unit_x.Collinear(v_unit_y));
}

TEST_F(VectorTest, CollinearWithZeroVector) {
    EXPECT_TRUE(v_1.Collinear(v_zero));
    EXPECT_TRUE(v_zero.Collinear(v_1));
    EXPECT_TRUE(v_zero.Collinear(v_zero));
}

TEST_F(VectorTest, CollinearOppositeDirection) {
    Vector v_1_opposite{-1.0, -2.0, -3.0};
    EXPECT_TRUE(v_1.Collinear(v_1_opposite));
}
